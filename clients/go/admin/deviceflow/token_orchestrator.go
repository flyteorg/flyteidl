...
// DeviceFlowTokenOrchestrator implements the main logic to initiate device authorization flow
type DeviceFlowTokenOrchestrator struct {
	cfg          Config
...
func (d DeviceFlowTokenOrchestrator) RefreshToken(ctx context.Context, token *oauth2.Token) (*oauth2.Token, error) {
	ts := d.clientConfig.TokenSource(ctx, token)
	...
	if refreshedToken != nil {
		...
		if refreshedToken.AccessToken != token.AccessToken {
			if err = d.tokenCache.SaveToken(refreshedToken); err != nil {
...
func (d DeviceFlowTokenOrchestrator) FetchTokenFromCacheOrRefreshIt(ctx context.Context) (token *oauth2.Token, err error) {
	token, err = d.tokenCache.GetToken()
	if err != nil {
		return nil, err
	}
	...
	if time.Now().Before(token.Expiry.Add(-d.cfg.TokenRefreshGracePeriod.Duration)) {
		//if token.Expiry.Add(f.cfg.TokenRefreshGracePeriod.Duration).Before(time.Now()) {
		return token, nil
	}
	token.Expiry = token.Expiry.Add(-d.cfg.TokenRefreshGracePeriod.Duration)

	token, err = d.RefreshToken(ctx, token)
	if err != nil {
	...
	err = d.tokenCache.SaveToken(token)
	if err != nil {
...
func (t DeviceFlowTokenOrchestrator) StartDeviceAuthorization(ctx context.Context, dareq DeviceAuthorizationRequest) (*DeviceAuthorizationResponse, error) {
...
func (d DeviceFlowTokenOrchestrator) PollTokenEndpoint(ctx context.Context, tokReq DeviceAccessTokenRequest, pollInterval int64) (*oauth2.Token, error) {
	v := url.Values{
	...
	for {
		...
		httpReq, err := http.NewRequest("POST", d.clientConfig.Endpoint.TokenURL, strings.NewReader(v.Encode()))
		if err != nil {
			return nil, err
		}
		httpReq.Header.Set("Content-Type", "application/x-www-form-urlencoded")
		...
		if len(tokResp.Error) > 0 {
			...
			err = d.tokenCache.SaveToken(&tokResp.Token)
...
func (d DeviceFlowTokenOrchestrator) FetchTokenFromAuthFlow(ctx context.Context) (*oauth2.Token, error) {
	ctx, cancelNow := context.WithTimeout(ctx, d.cfg.DeviceFlowTimeout.Duration)
	defer cancelNow()

	var scopes string
	if len(d.clientConfig.Scopes) > 0 {
		scopes = strings.Join(d.clientConfig.Scopes, " ")
	}
	daReq := DeviceAuthorizationRequest{ClientID: d.clientConfig.ClientID, Scope: scopes}
	daResp, err := d.StartDeviceAuthorization(ctx, daReq)
	if err != nil {
		return nil, err
	}

	pollInterval := int64(5) // default value of 5 sec poll interval if the authroization response doesn't have interval set
	if daResp.Interval > 0 {
		pollInterval = daResp.Interval
	}

	tokReq := DeviceAccessTokenRequest{ClientID: d.clientConfig.ClientID, DeviceCode: daResp.DeviceCode, GrantType: grantType}
	return d.PollTokenEndpoint(ctx, tokReq, pollInterval)
}
...
func NewDeviceFlowTokenOrchestrator(ctx context.Context, cfg Config, tokenCache cache.TokenCache, authMetadataClient service.AuthMetadataServiceClient) (DeviceFlowTokenOrchestrator, error) {
	clientConf, err := oauth.BuildConfigFromMetadataService(ctx, authMetadataClient)
	if err != nil {
		return DeviceFlowTokenOrchestrator{}, err
	}

	return DeviceFlowTokenOrchestrator{
		cfg:          cfg,
