...
/* 
AdminServiceApiService
Retrieves an eligible Execution to be processed by FlytePropeller.  The execution will be locked and provides strong transactional semantics. Only one processor will get one execution. Routing rules are used to determine is eligible
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param body

@return AdminRetrieveAndLockExecutionResponse
*/
func (a *AdminServiceApiService) RetrieveAndLockExecution(ctx context.Context, body AdminRetrieveAndLockExecutionRequest) (AdminRetrieveAndLockExecutionResponse, *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Post")
		localVarPostBody   interface{}
		localVarFileName   string
		localVarFileBytes  []byte
		localVarReturnValue AdminRetrieveAndLockExecutionResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/api/v1/executions/retrieve"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v AdminRetrieveAndLockExecutionResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"));
				if err != nil {
					newErr.error = err.Error()
					return localVarReturnValue, localVarHttpResponse, newErr
