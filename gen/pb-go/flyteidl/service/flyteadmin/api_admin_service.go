 ...
 * @param optional nil or *GetDataOpts - Optional Parameters:
     * @param "ArtifactFlyteUrl" (optional.String) - 

@return AdminDataResponse
*/

type GetDataOpts struct { 
	ArtifactFlyteUrl optional.String
}

func (a *AdminServiceApiService) GetData(ctx context.Context, localVarOptionals *GetDataOpts) (AdminDataResponse, *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Get")
		localVarPostBody   interface{}
		localVarFileName   string
		localVarFileBytes  []byte
		localVarReturnValue AdminDataResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/api/v1/data/get_artifact"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if localVarOptionals != nil && localVarOptionals.ArtifactFlyteUrl.IsSet() {
		localVarQueryParams.Add("artifact.flyte_url", parameterToString(localVarOptionals.ArtifactFlyteUrl.Value(), ""))
	}
	// to determine the Content-Type header
	localVarHttpContentTypes := []string{"application/json"}
