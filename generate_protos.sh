...
ls -d protos/docs/core/* | grep -v index.md | xargs rm
...
# Remove any currently generated file
ls -d protos/docs/admin/* | grep -v index.md | xargs rm
# Use the list to generate the MD files required for sphinx conversion
...
# Remove any currently generated file
ls -d protos/docs/datacatalog/* | grep -v index.md | xargs rm
# Use the list to generate the MD files required for sphinx conversion
...
# Remove any currently generated file
ls -d protos/docs/event/* | grep -v index.md | xargs rm
# Use the list to generate the MD files required for sphinx conversion
...
# Use the list to generate the MD files required for sphinx conversion
docker run --rm -v $(pwd)/protos/docs/plugins:/out -v  $(pwd)/protos:/protos pseudomuto/protoc-gen-doc --doc_opt=markdown,plugins.md -I=protos `echo $plugins_proto_files`
