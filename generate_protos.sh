...
docker run -v $DIR:/defs namely/protoc-all:1.11 -i ./protos -d protos/admin --with-gateway -l python
docker run -v $DIR:/defs namely/protoc-all:1.11 -i ./protos -d protos/core --with-gateway -l python
