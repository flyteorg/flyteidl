...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "refreshTokenPreemptively"), defaultConfig.RefreshTokenPreemptively, "Preemptively refreshes auth token before it expires (client credential only).")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "MaxDurationBeforeExpiry"), defaultConfig.MaxDurationBeforeTokenExpiry.String(), "Max duration between token refresh attempt and token expiry")
