...
type Config struct {
	...
	RefreshTokenPreemptively     bool            `json:"refreshTokenPreemptively" pflag:",Preemptively refreshes auth token before it expires (client credential only)."`
	MaxDurationBeforeTokenExpiry config.Duration `json:"MaxDurationBeforeExpiry" pflag:",Max duration between token refresh attempt and token expiry"`
...
var (
	defaultConfig = Config{
		...
		MaxDurationBeforeTokenExpiry: config.Duration{Duration: 10 * time.Minute},
